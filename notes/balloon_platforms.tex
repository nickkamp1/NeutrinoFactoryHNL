...
\begin{abstract}
We assess aerial platform options for a Cherenkov camera that would detect
...
balloons, high-altitude pseudo-satellites, station-keeping balloons, and
tethered aerostats, we conclude that a \textbf{tethered aerostat at
$\sim$5~km altitude} best satisfies the requirements of heavy payload
capacity, indefinite station-keeping, and pointing stability.  Combined with
a \textbf{horizontal beam dump geometry} that exploits Earth's curvature to
produce a $\sim$250~km decay baseline, this configuration offers a practical
path to a first-generation HNL search instrument.
\end{abstract}
...
(Section~\ref{sec:detector}--\ref{sec:platforms}), argues that a tethered
...
(Section~\ref{sec:geometry}), and outlines the remaining trade-off
studies (Section~\ref{sec:outlook}).
...
We consider four classes of aerial platforms.  Their key characteristics
...
\bottomrule
\end{tabular}
\caption{Comparison of aerial platform options.}
\label{tab:platforms}
\end{table}
