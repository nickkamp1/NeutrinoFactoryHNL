...
def muon_image_at_detector(decay_pos, mu_dir, mu_energy, det_pos,
                           ...
                           n_phase=300, apply_transmission=True):
...
def sample_dimuon_kinematics(geom, m_N, U2, detector_positions, N_samples=2000,
    ...
    Returns a dict of arrays (length N_samples):
        ...
        hnl_energy, decay_probability, valid_decay (bool), interaction_probability
        (scalar), BR_mumu (scalar).
    The physics is identical to compute_dimuon_signal_at_satellite; only the
    Cherenkov signal is replaced by the imaging analysis downstream.
    """
    ...
    return dict(decay_points=decay_points, dir_mu1=dir_mu1, dir_mu2=dir_mu2,
                ...
                decay_probability=decay_probability, valid_decay=valid_decay,
...
def analyze_mass_mixing(geom, m_N, U2, detector_positions, N_samples=2000,
    ...
    for i in valid_idx:
        print(f"Processing event {i} out of {len(valid_idx)}...", end='\r')
        ...
        cols["weight"].append(kin["decay_probability"][i] * cherenkov_weight)
